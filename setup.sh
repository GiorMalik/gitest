...
# Auto-install jq if missing (used by many playbooks)
if ! command -v jq &>/dev/null; then
  echo "📦 jq not found — installing via apt..."
  apt-get install -y -qq jq 2>/dev/null && echo "✅ jq installed" || echo "⚠️  jq install failed — install manually: apt-get install jq"
fi
