...
### Sync .gitest state back to main repo

Before removing the engagement workspace, copy `$OUTDIR/` state back. When `$OUTDIR/` is gitignored, files written there during engagement workspace execution are not committed or submitd — they would be lost on engagement workspace removal.

```bash
# Sync .gitest state from engagement workspace to main repo (preserves task state, plans, notepads)
if [ -d "$WORKTREE_PATH/.gitest" ]; then
  mkdir -p "$ORIGINAL_DIR/.gitest"
  cp -r "$WORKTREE_PATH/$OUTDIR/"* "$ORIGINAL_DIR/$OUTDIR/" 2>/dev/null || true
fi
```
