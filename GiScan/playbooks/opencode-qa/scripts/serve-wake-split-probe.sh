...
#
# Proves whether gitest's plugin-origin promptAsync (parent-wake bg notifications)
...
#
# Env:
#   GITEST_SANDBOX_GITEST_CONFIG   JSON string; when set, deep-merged over the base
#                            agent overrides (env keys win) and written to
#                            $XDG_CONFIG_HOME/opencode/gitest-openagent.json
...
# Write the sandbox gitest config: base agent overrides (explore/librarian -> the
# fake provider, required for child model resolution) deep-merged with
# GITEST_SANDBOX_GITEST_CONFIG when set (jq '.[0] * .[1]'; env keys win).
# Args: sandbox_config_dir
swsp_write_gitest_config() {
  local cfg_dir="$1"
  local gitest_cfg="$cfg_dir/opencode/gitest-openagent.json"
  local base='{"agents":{"explore":{"model":"openai/gpt-fake"},"librarian":{"model":"openai/gpt-fake"}}}'

  mkdir -p "$cfg_dir/opencode"
  if [ -n "${GITEST_SANDBOX_GITEST_CONFIG:-}" ]; then
    if ! printf '%s\n%s\n' "$base" "$GITEST_SANDBOX_GITEST_CONFIG" | jq -s '.[0] * .[1]' >"$gitest_cfg" 2>/dev/null; then
      swsp_log "FAIL: GITEST_SANDBOX_GITEST_CONFIG is not valid JSON"
      return 1
    fi
    swsp_info "wrote merged GITEST_SANDBOX_GITEST_CONFIG to $gitest_cfg"
  else
    printf '%s\n' "$base" >"$gitest_cfg"
    swsp_info "wrote agent overrides to $gitest_cfg"
  fi
}
...
{
  "plugin": ["file://${repo_root}/packages/gitest-core/src/index.ts"],
...
# Count plugin_inits from the gitest log since a byte offset, filtered to sandbox dir.
# Args: log_offset sandbox_dir
swsp_count_plugin_inits() {
  local offset="$1"
  local sandbox_dir="$2"
  local log_path="${TMPDIR:-/tmp}/gitest.log"
  if [ ! -f "$log_path" ]; then
    printf '0'
    return 0
  fi
...
# true iff the gitest log (since offset) contains a [prompt-async-gate] promptAsync dispatching
...
swsp_detect_wake_during_parent() {
  ...
  local fake_log="$2"
  local sandbox_dir="$3"
  local gitest_log="${TMPDIR:-/tmp}/gitest.log"
  ...
  local dispatch_line
  dispatch_line="$(tail -c "+$((offset + 1))" "$gitest_log" 2>/dev/null \
...
swsp_self_test() {
  ...
  if ! oqa_start_server; then
    ...
    # GITEST_SANDBOX_GITEST_CONFIG env contract assertion: merge keeps base overrides
    local gitest_cfg_path="$XDG_CONFIG_HOME/opencode/gitest-openagent.json"
    GITEST_SANDBOX_GITEST_CONFIG='{"_probe":true}' swsp_write_gitest_config "$XDG_CONFIG_HOME"
    local probe_val explore_model
    probe_val="$(jq -r '._probe' "$gitest_cfg_path" 2>/dev/null)"
    explore_model="$(jq -r '.agents.explore.model' "$gitest_cfg_path" 2>/dev/null)"
    if [ "$probe_val" = "true" ] && [ "$explore_model" = "openai/gpt-fake" ]; then
      swsp_info "PASS: GITEST_SANDBOX_GITEST_CONFIG merge assertion (env key + base overrides both present)"
    else
      swsp_log "FAIL: gitest config merge wrong: _probe='$probe_val' explore_model='$explore_model'"
      fails=$((fails+1))
    fi
  fi
...
swsp_run_probe() {
  ...
  # Capture gitest log byte offset
  local gitest_log="${TMPDIR:-/tmp}/gitest.log"
  local gitest_log_offset
  if [ -f "$gitest_log" ]; then
    gitest_log_offset="$(wc -c <"$gitest_log" 2>/dev/null | tr -d ' ')" || gitest_log_offset=0
  else
    gitest_log_offset=0
  fi
  ...
  swsp_write_gitest_config "$XDG_CONFIG_HOME"
  ...
  prompt_response="$(curl -sS -u "opencode:${pass}" \
    ...
    -H 'content-type: application/json' \
    -d '{"parts":[{"type":"text","text":"Run the split probe: call call_gitest_agent exactly once as instructed, then run the bash hold command."}]}' \
  ...
  local plugin_inits
  plugin_inits="$(swsp_count_plugin_inits "$gitest_log_offset" "$OQA_PROJ")"
  ...
  local route_prov=""
  if [ -f "$gitest_log" ]; then
    route_prov="$(tail -c "+$((gitest_log_offset + 1))" "$gitest_log" 2>/dev/null \
  ...
  wake_during_parent="$(swsp_detect_wake_during_parent "$gitest_log_offset" "$fake_llm_log" "$OQA_PROJ")"
