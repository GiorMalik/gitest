...
`qa-docker.sh` brings up a disposable box (builds `gitest-dev` then `gitest-qa` on
...
`gitest-qa` is `gitest-dev` (`.devcontainer/Dockerfile`) plus the latest `opencode-ai`
and `@openai/codex` npm packages and `sqlite3 jq curl rsync`. Pin with
`--build-arg GITEST_OPENCODE_VERSION=...` on the qa.Dockerfile for a specific release.
...
Each run auto-removes its container (`--rm`). The `gitest-dev` / `gitest-qa` images
