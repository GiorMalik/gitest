...
2. **Create worktree** in sibling directory:
   ```bash
   mkdir -p ../gitest-wt
   git worktree add ../gitest-wt/feat-max-background-agents feat/max-background-agents
   ```

3. **All subsequent work** happens in `../gitest-wt/feat-max-background-agents/`, never in the main worktree.
...
2. **Remove worktree:**
   ```bash
   git worktree remove ../gitest-wt/feat-max-background-agents
   ```
