...
# structured notification stream. This is how you prove the gitest plugin behaves
...
#                 Fast; no install.
#   --plugin      Install THIS repo's local gitest build into the isolated home,
...
#
# The captured JSON summary IS the evidence; redirect it into $OUTDIR/evidence/.
...
cqa_drive() {
  ...
  if [ "$plugin" = "1" ]; then
    cqa_log "installing local gitest build into $CODEX_HOME (this builds the plugin)..."
    if ! cqa_install_local_gitest; then
      cqa_log "install failed; tail:"; tail -20 "$CQA_HOME_ROOT/install.log" >&2 2>/dev/null
      return 1
    fi
    grep -q 'gitest@local' "$CODEX_HOME/config.toml" || { cqa_fail "gitest not enabled in isolated config.toml"; return 1; }
  fi
