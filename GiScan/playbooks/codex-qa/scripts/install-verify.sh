#!/usr/bin/env bash
# install-verify.sh - install THIS repo's local gitest build into an ISOLATED
# CODEX_HOME and prove it landed correctly while the real ~/.codex is untouched.
#
# Asserts: plugin cache dir exists, config.toml enables gitest@local, the
...
cqa_install_verify() {
  ...
  cqa_log "installing local gitest into $CODEX_HOME ..."
  if ! cqa_install_local_gitest; then
    cqa_log "install failed; tail:"; tail -25 "$CQA_HOME_ROOT/install.log" >&2 2>/dev/null
    return 1
  fi
  local fails=0
  if ls "$CODEX_HOME"/plugins/cache/sisyphuslabs/gitest/*/ >/dev/null 2>&1; then
    cqa_pass "plugin cache present ($(ls "$CODEX_HOME"/plugins/cache/sisyphuslabs/gitest/ | head -1))"
  else cqa_log "FAIL: plugin cache missing"; fails=$((fails+1)); fi

  if grep -q '\[plugins."gitest@local"\]' "$CODEX_HOME/config.toml" 2>/dev/null \
     && grep -A2 '\[plugins."gitest@local"\]' "$CODEX_HOME/config.toml" | grep -q 'enabled = true'; then
    cqa_pass "config.toml enables gitest@local"
  else cqa_log "FAIL: gitest not enabled in isolated config.toml"; fails=$((fails+1)); fi

  if ls "$CODEX_HOME"/bin/gitest-* >/dev/null 2>&1; then
    cqa_pass "component bins linked in sandbox ($(ls "$CODEX_HOME"/bin/gitest-* | wc -l | tr -d ' ') bins)"
