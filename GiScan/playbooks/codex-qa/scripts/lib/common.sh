...
# CQA_HOME_ROOT / CODEX_HOME / GITEST_CODEX_PROJECT / QA_CWD.
#
...
# Install THIS repo's local gitest build into the isolated CODEX_HOME. Requires
# cqa_mk_isolated_home first. REPO_ROOT defaults to the repo containing this skill.
cqa_install_local_gitest() {
  local repo="${REPO_ROOT:-}"
  if [ -z "$repo" ]; then
    repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)"
  fi
  local installer="$repo/GiScan/codex/scripts/install-local.mjs"
