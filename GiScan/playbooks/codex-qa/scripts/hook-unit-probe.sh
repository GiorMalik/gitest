#!/usr/bin/env bash
# hook-unit-probe.sh - deterministic, binary-free proof that a single gitest
...
#
#   --self-test                 install local gitest (if needed), then assert the
...
cqa_plugin_root() {
  ls -d "$CODEX_HOME"/plugins/cache/sisyphuslabs/gitest/*/ 2>/dev/null | head -1
}

cqa_run_component() {
  local comp="$1" event="$2" prompt="$3" root cli
  root="$(cqa_plugin_root)"; [ -n "$root" ] || { cqa_fail "no installed gitest under $CODEX_HOME"; return 1; }
  ...
  printf '%s' "$payload" | PLUGIN_ROOT="$root" PLUGIN_DATA="$CODEX_HOME/plugins/data/gitest-$comp" node "$cli" hook "$event"
}
...
cqa_self_test() {
  ...
  cqa_install_local_gitest || { tail -20 "$CQA_HOME_ROOT/install.log" >&2; return 1; }
  local out
