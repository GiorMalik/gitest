...
- **The captured JSON / pane IS the evidence** — write it under
  `$OUTDIR/evidence/<YYYYMMDD>-<slug>/` (no evidence file == the QA did not happen).
...
| `scripts/app-server-drive.sh` | `--self-test`: a bare turn completes and the mock assistant text comes back. `--plugin`: installs local gitest, drives a turn, and asserts `hook/completed` for `sessionStart,userPromptSubmit` |
| `scripts/install-verify.sh` | local gitest installs into the isolated home; `config.toml` enables `gitest@local`; component bins + agent TOMLs linked in the sandbox; real `~/.codex` unchanged |
...
- **Component / hook logic** (`GiScan/codex/plugin/components/*`):
...
- **Installer / config.toml** (`GiScan/codex/src/install/*`):
  `install-verify.sh`.
...
```bash
ev="$OUTDIR/evidence/$(date +%Y%m%d)-codex-qa-<slug>"; mkdir -p "$ev"
