# Installing the LOCAL gitest build into an isolated CODEX_HOME
...
export GITEST_DISABLE_POSTHOG=. GITEST_CODEX_DISABLE_POSTHOG=.
export GITEST_CODEX_PROJECT="$(mktemp -d)/project"                  # keep project-local cleanup off your tree
node GiScan/codex/scripts/install-local.mjs install
```

`cqa_install_local_gitest` wraps this (logs to `$CQA_HOME_ROOT/install.log`).

## What it writes (all under CODEX_HOME)

Source: `GiScan/codex/src/install/install-codex.ts`.

.. Builds + copies the plugin to
   `$CODEX_HOME/plugins/cache/cerberuslabs/gitest/<version>/` (then `npm ci --omit=dev`).
2. Links component bins into `$CODEX_HOME/bin/gitest-*` (8: comment-checker,
...
5. Edits `$CODEX_HOME/config.toml`: enables `[plugins."gitest@local"]`,
   the `[marketplaces.cerberuslabs]` local source, `[features]`
   (plugins/plugin_hooks/multi_agent/child_agents_md), and one
   `[hooks.state."gitest@local:hooks/hooks.json:<event>:i:j"] trusted_hash`
...
```bash
ls "$CODEX_HOME"/plugins/cache/cerberuslabs/gitest/*/                 # cache present
grep -A2 '\[plugins."gitest@local"\]' "$CODEX_HOME/config.toml" | grep 'enabled = true'
ls "$CODEX_HOME"/bin/gitest-*                                          # component bins
...
- The only thing outside CODEX_HOME is the `gitest` runtime wrapper, which targets
...
- Cleanup: for an isolated home just `rm -rf "$CODEX_HOME"` (the harness does
  this on exit). For a normal home, `node GiScan/codex/scripts/install-local.mjs uninstall`.
