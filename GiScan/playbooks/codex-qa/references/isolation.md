...
The whole point of this skill: exercise the gitest plugin in a real Codex without
...
- `GITEST_CODEX_PROJECT` + `QA_CWD` → a sandbox project dir, so the installer's
...
- `GITEST_DISABLE_POSTHOG=.` + `GITEST_CODEX_DISABLE_POSTHOG=.` → no install/telemetry
  network call.
